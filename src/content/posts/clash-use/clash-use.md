...
我购买了一条typec转RJ45的千兆网线，意图提升我NAS备份照片的速度时（这个NAS也很有意思，在很久之后或许我会单开一篇博文讲）。然后我发现，似乎我的网速并不高，不论是连5G还是直连网线，上下传速度在90Mbps，这个是典型的百兆网再加传输损失的网速（直接连校园网的WiFi都有120Mbps）。那么问题可能出在哪里呢

1. 网线降级了，比如损坏或协议问题。这个新买的一般不可能。
2. 路由器LAN口带宽不高。GL.inet MT-3000的LAN口在2.5Gbps，所以这个也不是原因。
3. OpenClash不是走硬件加速，导致速度有问题。

然后，我就连上后（我发现我新电脑还没网口了，只能连WAN口），把路由器从路由器模式设置成AP模式了，相当于一个交换机，这样就排除了OpenClash的影响。然而其实问题在于

4. 工位被限速了。可能是针对账号的限速或者工位的网口就是被限速了，我工位上测速还是90Mbps。

那么这样一来直连也无意义了。但是上述操作却带来了一个问题，本来`192.168.8.1`是我的网关（从ipconfig可查），也是路由器的后台地址，现在换成AP模式后却不是了。AP直接桥接了学校上游，而学校的网关又打不开，我无法查阅现在路由器的后台地址了。于是我用
```shell
nmap -sn A.B.C.0/23   # 学校网段
```
暴力扫描一下，有258个活跃连接点。但是太多了，我于是发给deepseek让他排查下，他马上找到了。幸好他设备名比较有标识，不是什么`(New H3C Technologies)`。
```haskell
Nmap scan report for A.B.C.D
Host is up (0.0010s latency).
MAC Address: A:B:C:D:E:F (GL Technologies (Hong Kong) Limited)
```

现在终于可以登入后台改回路由器模式了。不然恢复出厂设置，配置的openclash就都没了。
